...
- (IBAction)stopRecord:(id)sender {
   ...
   [self removeTimer];
    ...
    NSString *path = [ lameTool audioToMP3:filePath isDeleteSourchFile:NO];
    
    
    NSFileManager *manager = [NSFileManager defaultManager];
    if ([manager fileExistsAtPath:filePath]){
        
        _noticeLabel.text = [NSString stringWithFormat:@"录了 %ld 秒,文件大小为 %.2fKb",COUNTDOWN - (long)countDown,[[manager attributesOfItemAtPath:path error:nil] fileSize]/1024.0];
